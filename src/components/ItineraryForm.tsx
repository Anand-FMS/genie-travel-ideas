...
export default ItineraryForm;import { useState } from "react";
import { useNavigate } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
...
const ItineraryForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const aiText =
        data?.content?.[0]?.parts?.[0]?.text || // Gemini 2.5 JSON output
        data?.parts?.[0]?.text ||
        data?.text ||
        JSON.stringify(data);

      // Save raw JSON for Results.tsx
      sessionStorage.setItem("generatedItinerary", aiText);
      ...
      console.error("Error generating itinerary:", error);
      alert("Could not connect to AI server. Check if n8n is running.");
    }
  };
